...
class IheModuleSvm(Svm):
    ...
    def print_text_prediction(self, text) -> None:
        """
            Predicts probabilities of IHEs given any random text input.
        """
    ...
    def run(self) -> None:
        """
            Trains the SVM model for clasifying IHEs using stochastic gradient descent.
        """
        ts = time.time()
        startTime = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        
        svm_dataset = "main/NLP/SVM/dataset.csv"
        tags = ['IHE {}'.format(i) for i in range(1, 19)] # IHE tags.

        # SDG results files.
        model = "main/NLP/SVM/IHE_MODULES_RESULTS/model.pkl"
        results = "main/NLP/SVM/IHE_MODULES_RESULTS/training_results.json"
