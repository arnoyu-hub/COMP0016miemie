...
class HaSvmDataset():
    ...
    def __init__(self):
        ...
        self.svm_dataset = "main/NLP/SVM/SVM_dataset_ha.csv"
        ...
        self.num_has = len(self.data['Topic Words'])
    ...
    def tag_publications(self) -> pd.DataFrame:
        """
            Returns a dataframe with columns {ID, Description, HA} for each publication, where SDG is a class tag for training the SVM.
        """
        ...
        for doi in self.data['Document Topics']:
            self.__progress(counter, num_publications, "Forming Publications HA Dataset for SVM...")
