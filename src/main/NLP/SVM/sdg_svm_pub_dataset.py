...
class SdgSvmPubDataset():
    ...
    def __init__(self):
        ...
        self.svm_dataset = "main/NLP/SVM/SVM_dataset_sdg.csv"

        with open("main/NLP/LDA/SDG_PUB_RESULTS/training_results.json") as json_file:
           self.data = json.load(json_file)

        # dataframe with columns {DOI, Title, Description}.
        self.df_publications = self.publication_loader.load("MAX")
        self.num_sdgs = len(self.data['Topic Words'])
    ...
    def tag_publications(self) -> pd.DataFrame:
        ...
        for doi in self.data['Document Topics']:
            self.__progress(counter, num_publications, "Forming Publications SDG Dataset for SVM...")
