...
class IHESvmModuleDataset():
    ...
    def get_module_description(self, doi: str):
        """
            Returns the module description for a particular DOI.
        """

        # search for row in dataframe by DOI.
        df = self.df_modules.loc[self.df_modules["Module_ID"] == doi]
    ...
    def tag_modules(self) -> pd.DataFrame:
        ...
        num_modules = len(self.data['Document Topics'])
        final_data = {}
        counter = 0

        for doi in self.data['Document Topics']:
            self.__progress(counter, num_modules, "Forming Modules IHE Dataset for SVM...")
            ...
            description = self.get_module_description(doi)
