"""
    Standalone file to analyse the direct results of SDG_PUB_LDA and output the authors for each publication under each SDG speciality
"""
...
def get_results(file_name: str) -> dict:
    """
        Returns JSON file storing LDA results for SDG publication classifications
    """
...
def get_authors(query_result: list, results: dict, approach: int, cols: list, ucl_only: bool) -> dict:
    """
        Run main analysis to extract authors for each SDG field
    """
    ...
    for items in query_result:
        if items[0]['DOI'] in results['Document Topics']:
            assigned_SDG = results['Document Topics'][items[0]['DOI']]
            author_data = items[0]['AuthorData']

            sdg_string = items[1]['SDG_Approach_String'] if 'SDG_Approach_String' in items[1] else None
            approach = str(approach)
            if (approach != '0') and (not (sdg_string and (approach in sdg_string))):
                continue
            already_classified = False
            for i, val in enumerate(assigned_SDG):
                temp = float(val[4:-2])
                if temp >= THRESHOLD:
...
def main() -> None:
    """
        ...
        Produces SDG_Case_Study_regmed_tisseng_RENAL.csv file for LDA results analysis
    """

    input_file = "main/NLP/LDA/SDG_PUB_RESULTS/training_results_all.json"
    output_file = "SDG_Case_Study_new.csv"
