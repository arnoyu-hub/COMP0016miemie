"""
    Standalone file to analyse the direct results of HA_LDA and output the authors for each publication under each HA speciality
"""
...
def get_results(file_name: str) -> dict:
    """
        Returns JSON file storing LDA results for HA publication classifications
    """
...
def get_authors(query_result: list, results: dict, approach: int, cols: list, ucl_only: bool) -> dict:
    """
        Run main analysis to extract authors for each HA field
    """
    ...
    for items in query_result:
        if items[0]['DOI'] in results['Document Topics']:
            assigned_HA = results['Document Topics'][items[0]['DOI']]
            author_data = items[0]['AuthorData']

            ha_string = items[1]['HA_Approach_String'] if 'HA_Approach_String' in items[1] else None
            approach = str(approach)
            if (approach != '0') and (not (ha_string and (approach in ha_string))):
                continue
            already_classified = False
            for i, val in enumerate(assigned_HA):
                temp = float(val[4:-2])
                if temp >= THRESHOLD:
...
def main() -> None:
    """
        ...
        Produces HA_Case_Study_regmed_tisseng_RENAL.csv file for LDA results analysis
    """

    input_file = "main/NLP/LDA/HA_RESULTS/training_results_all.json"
    output_file = "HA_Case_Study_new.csv"
